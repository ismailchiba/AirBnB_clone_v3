...
def text_if_int(n):
    """script that display text only if int given"""
...
def html_if_stateID(id):
    """script that display html page; customize heading with state.name
...
def html_filters():
    """script that display html page with working city/state filters & amenities
...
def html_all_filters():
    """script that display html page w/ working city/state filters & amenities/properties
       runs with web static css files
    """
    state_objs = [s for s in storage.all("State").values()]
    amenity_objs = [a for a in storage.all("Amenity").values()]
