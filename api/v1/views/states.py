...
def list_of_states():
    ...
    states_lists = [obj.to_dict() for obj in storage.all("State").values()]
    return jsonify(states_lists)


@app_views.route('/states/<state_id>', methods=['GET'])
def get_state(state_id):
    '''Retrieves a State object'''
    current_state = storage.all("State").values()
    state_obj = [obj.to_dict() for obj in current_state if obj.id == state_id]
    if state_obj == []:
        abort(404)
