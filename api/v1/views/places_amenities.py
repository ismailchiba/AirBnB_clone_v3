...
def deletes_a_amenity(amenity_id, place_id):
    '''Deletes a Amenity object '''
    all_amenities = storage.all("Amenity").values()
    ...
    for obj in all_places:
        if obj.id == place_id:
            if all_places == []:
                abort(404)
        place_obj = obj.to_dict()

    if amenity not in place_obj.amenity_obj:
        abort(404, {Amenity not linked to this place})
    
    place_obj.amenity_obj.remove(amenity)
    storage.save()
    return jsonify({}), 200

@app_views.route('/amenities/<amenity_id>', methods=['DELETE'])
def links_a_amenity(amenity_id, place_id):
    ...
    if amenity in place_obj.amenity_obj:
        return jsonify(amenity.to_dict()), 200
    
    place_obj.amenity_obj.append(amenity)
    storage.save()
