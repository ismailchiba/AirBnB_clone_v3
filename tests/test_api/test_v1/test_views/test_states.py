#!/usr/bin/python3
'''script that test the index route'''
import unittest
import pep8
...
class TestStates(unittest.TestCase):
    '''script that test state'''
    def test_lists_states(self):
        '''script that test state GET route'''
    ...
    def test_create_state(self):
        '''script that test state POST route'''
        with app.test_client() as c:
    ...
    def test_delete_state(self):
        '''script that test state DELETE route'''
        with app.test_client() as c:
    ...
    def test_get_state(self):
        '''script that test state GET by id route'''
        with app.test_client() as c:
            new_state = State(name="Beckystan")
            storage.new(new_state)
            resp = c.get('api/v1/states/{}'.format(new_state.id))
    ...
    def test_update_state(self):
        '''script that test state PUT route'''
        with app.test_client() as c:
            new_state = State(name="Beckystan")
            storage.new(new_state)
