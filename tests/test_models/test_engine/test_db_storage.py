...
class TestDBStorage(unittest.TestCase):
    ...
    def test_new(self):
        ...
        rtriv_state = session.query(State).filter_by(id=new_state.id).first()

        self.assertEqual(rtriv_state.id, new_state.id)
        self.assertEqual(rtriv_state.name, new_state.name)
        self.assertIsNotNone(rtriv_state)
    ...
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_get(self):
        """Test that get obtains an instance db storage"""
        storage = models.storage

        storage.reload()

        state_data = {"name": "Minnesota"}
    ...
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_count(self):
        """Test that count obtains an instance db storage"""
        storage = models.storage
        storage.reload()
        state_data = {"name": "Kansas"}
        state_instance = State(**state_data)
        storage.new(state_instance)

        city_data = {"name": "Calabasas", "state_id": state_instance.id}
