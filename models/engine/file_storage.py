...
class FileStorage:
    ...
    def new(self, obj):
        """sets objects"""
        _id = "{}.{}".format(type(obj).__name__, obj.id)
    ...
    def save(self):
        ...
        d = {}
        for _id, bm_obj in FileStorage.__objects.items():
            d[_id] = bm_obj.to_json()
    ...
    def delete(self, obj=None):
        ...
        if obj is None:
            return
        for l in list(FileStorage.__objects.keys()):
            if obj.id == l.split(".")[1] and l.split(".")[0] in str(obj):
                FileStorage.__objects.pop(l, None)
                self.save()
